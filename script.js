...
window.addEventListener('DOMContentLoaded', async () => {
  try {
    if (chapterId) {
      ...
      await loadMangaDetails(mangaId); // FIXED
    }
  } catch (error) {
...
function displayMangaDetails(data) {
  ...
  document.getElementById('manga-authors').textContent = manga.relationships?.find(r => r.type === 'author')?.attributes?.name || 'Unknown';
